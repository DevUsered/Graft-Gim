...
export default function SuperAdmin() {
  ...
  const cargarGimnasios = () => {
    fetch('http://localhost:8080/api/superadmin/gimnasios', {
  ...
  const registrarGimnasio = async (e: React.FormEvent) => {
    ...
    setCargando(true);
    try {
      const res = await fetch('http://localhost:8080/api/superadmin/gimnasios', {
        method: 'POST',
  ...
  const cambiarEstado = async (id: number, estadoActual: string) => {
    ...
    try {
      await fetch(`http://localhost:8080/api/superadmin/gimnasios/${id}/estado`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('token') },
  ...
  const renovarSuscripcion = async (id: number, nombre: string) => {
    ...
    try {
      await fetch(`http://localhost:8080/api/superadmin/gimnasios/${id}/renovar`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('token') },
  ...
  const enviarAviso = async (id: number) => {
    ...
    try {
      await fetch(`http://localhost:8080/api/superadmin/gimnasios/${id}/recordatorio`, {
        method: 'POST',
        headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') }
      });
